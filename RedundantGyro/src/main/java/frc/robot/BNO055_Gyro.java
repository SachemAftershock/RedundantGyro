/** This class was cloned and owened from the work of FRC Team 2168
 * https://github.com/jcorcoran/BNO055_FRC.git
 */
...
public class BNO055_Gyro {
	...
	private static BNO055_Gyro instance;
	 ...
	 */
	private BNO055_Gyro(I2C.Port port, byte address) {
	 ...
	 */
	public static BNO055_Gyro getInstance(opmode_t mode, vector_type_t vectorType,
			I2C.Port port, byte address) {
		if(instance == null) {
			instance = new BNO055_Gyro(port, address);
		}
		requestedMode = mode;
...
//					+ ", next: " + nextTime);
	...
	public double getHeadingRangedLikeNavX() {
		//final double x1 = 0, x2 = 360, y1 = -180, y2 = 180, m = (y2-y1)/(x2-x1), b = y2-m*x2;
		//return m * xyz[0] + b;
		double workingValue = xyz[0];
		if (workingValue > 180.0) workingValue = -180.0 + (workingValue-180.0);
		if (workingValue < -180.0) workingValue = 180.0 + (workingValue+180.0);
		return workingValue;
	}
	...
	private class BNO055UpdateTask extends TimerTask {
		private BNO055_Gyro imu;

		private BNO055UpdateTask(BNO055_Gyro imu) {
			if (imu == null) {
	...
	public void BNO055_Init() {
		System.out.println("=======================================");
//		BNO055_Gyro.getInstance(BNO055_Gyro.opmode_t.OPERATION_MODE_IMUPLUS,
//		BNO055_Gyro.vector_type_t.VECTOR_EULER);
		System.out.println("BNO055: Rev Info: " + getRevInfo().toString());
		System.out.println("=======================================");
	}

	public void BNO055_Periodic() {
		System.out.println("BNO055: Rev Info / Gyro Rev: " + getRevInfo().gyro_rev);
		System.out.println("BNO055: Rev Info / SW Rev: " + getRevInfo().sw_rev);
		System.out.println("BNO055: Subsystem Status / Self Test Result: " + getSystemStatus().self_test_result);
		System.out.println("BNO055: Subsystem Status / System Error: " + getSystemStatus().system_error);
		System.out.println("BNO055: Subsystem Status / System Status: " + getSystemStatus().system_status);
		System.out.println("BNO055: Initialized " + isInitialized());
		System.out.println("BNO055: Sensor Present " + isSensorPresent());
		System.out.println("BNO055: Calibrated " + isCalibrated());
		System.out.println("BNO055: Calibration.sys " + getCalibration().sys);
		System.out.println("BNO055: Calibration.gyro " + getCalibration().gyro);
		System.out.println("BNO055: Calibration.mag " + getCalibration().mag);
		System.out.println("BNO055: GetHeading: " + getHeading());
		System.out.println("BNO055: GetVector[0]: " + getVector()[0]);
		System.out.println("BNO055: GetVector[1]: " + getVector()[1]);
		System.out.println("BNO055: GetVector[2]: " + getVector()[2]);
	}
